def return_instruction_detailed_inverter_performance() -> str:
    ...
    - **device_id**: List of inverter device IDs to analyze
    - **dates**: Corresponding dates to check for each device
    - **Example**: {"device_A": ["2025-04-25", "2025-04-05"], "device_B": ["2025-04-20"]}
    ...
    - **active_power_kw**: Real-time active power output in kW
    - **ava_inv_status**: Inverter availability status
    - **daily_yield_kwh**: Daily energy production in kWh
    - **daily_operation_time_hours**: Hours of operation per day
    - **daily_irradiation_effect_kwhm_squared**: Daily irradiation impact
