...
def initial_config(
    ...
    dictionary device id and capacity peak : id for the inverters of the specific plants and its capacity peak , kilowatt peak .Example: {'abcdefg':'130.5kwp'}
    list date : list of dates requested to check
    """
    callback_context.state["plant_id"] = plant_id
    callback_context.state["inverter_date_to_check"] = inverter_date_to_check
    callback_context.state["inverter_device_id_and_capacity_peak"] = (
        inverter_device_id_and_capacity_peak
    )
    callback_context.state["date_requested"] = date_requested
