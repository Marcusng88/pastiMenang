...
# def filter_plant_timeseries_data(string_data : str, tool_context : ToolContext):
#     """
...
#     # Derived Features
#     df_work['norm_power'] = df_work['active_power_effective_kw'] / df_work['irradiance_wm_squared'].replace(0, pd.NA)
#     df_work['smoothed_power'] = df_work['active_power_effective_kw'].rolling(window=12, min_periods=1).mean()

#     # Rule-based Detection
#     df_work['low_yield'] = (df_work['irradiance_wm_squared'] > 400) & (df_work['five_min_pr_percent'] < 60)
#     df_work['power_drop'] = df_work['active_power_effective_kw'].diff() < -100
#     df_work['clipping'] = (df_work['active_power_effective_kw'].diff().abs() < 1) & (df_work['irradiance_wm_squared'] > 900)

#     # Time Series Decomposition
#     try:
#         result = seasonal_decompose(df_work['active_power_effective_kw'].interpolate(),
...
#     # ML-Based Detection
#     ml_features = df_work[['norm_power', 'five_min_pr_percent', 'pv_module_temperature_c']].dropna()
...
#         df_work['ml_anomaly'] = df_work['ml_anomaly'].fillna(False)
#     else:
#         df_work['ml_anomaly'] = False

#     # Combine flags
#     df_work['production_final'] = df_work[['low_yield', 'power_drop', 'clipping',
#                                          'residual_anomaly', 'ml_anomaly']].any(axis=1)
...
#         pr_mask = (df_work['irradiance_wm_squared'] > 400) & (df_work['five_min_pr_percent'] < 70)
...
#         temp_outlier = df_work['pv_module_temperature_c'] > df_work['pv_module_temperature_c'].quantile(0.95)
...
#     filtered_df = hybrid_anomalies[['five_min_pr_percent','active_power_effective_kw','five_min_pr_percent','irradiance_wm_squared','pv_module_temperature_c']]

#     # save to tool context
#     tool_context.state['filtered_plant_timeseries_df'] = filtered_df
#     tool_context.state['production_anomalies'] = production_anomalies
#     tool_context.state['exploratory'] = hybrid_anomalies
#     tool_context.state['full_data'] = df_work
...
async def filter_plant_timeseries_data(
    ...
    df_work["power_drop"] = df_work["active_power_effective_kw"].diff() < -100
    df_work["clipping"] = (df_work["active_power_effective_kw"].diff().abs() < 1) & (
    ...
    if len(ml_features) > 10:
        ...
        df_work["ml_anomaly"] = (
            df_work["ml_anomaly"].fillna(False).infer_objects(copy=False)
        )
    else:
        df_work["ml_anomaly"] = False
