def return_instruction_coding() -> str:
    ...
    ## Core Requirements
    - Convert all data from `{daily_pr_agent_output}` `{detailed_plant_timeseries_agent_output}` `{detailed_inverter_performance_agent_output}` into HTML/CSS
    ...
    <!-- All data from `{daily_pr_agent_output}` `{detailed_plant_timeseries_agent_output}` `{detailed_inverter_performance_agent_output}` must be included -->
    </div>
    ```
